...
function assign(
  ...
  let best: { qty: number; unit: number; score: number } | null = null;
  ...
  if (explicitQty && looksLikeQuantity(explicitQty)) {
    if (positive.length === 1) return { qty: explicitQty, unit: positive[0] };
    ...
    return { qty: explicitQty, unit };
  }

  if (positive.length === 2) {
    const [a, b] = positive;
    if (looksLikeQuantity(a) && b > 0) return { qty: Math.max(1, Math.round(a)), unit: b };
    if (looksLikeQuantity(b) && a > 0) return { qty: Math.max(1, Math.round(b)), unit: a };
    return { qty: 1, unit: Math.max(a, b) };
  }

  return { qty: 1, unit: positive[positive.length - 1] };
}

function build(name: string, a: { qty: number; unit: number }): ParsedItem | null {
  if (!(a.unit > 0)) return null;
  return {
    name: name.slice(0, 60),
    quantity: Math.max(1, Math.min(99, a.qty)),
    unitPriceCents: Math.round(a.unit * 100)
  };
}
...
function parseLine(raw: string): ParsedItem | null {
  ...
  if (cols.length >= 2) {
    ...
    if (numCols.every(isNumericToken) && hasEnoughLetters(nameCol)) {
      ...
      const a = assign(effective, qtyInName?.qty);
      if (a) return build(nameCol, a);
    }
  }

  return parseFreeform(collapsed);
}

function parseFreeform(body: string): ParsedItem | null {
  ...
  const a = assign(nums, explicitQty);
  if (!a) return null;
  return build(name, a);
}
...
export function parseReceiptText(rawText: string): ParsedItem[] {
  ...
  if (parsed.length === 0 && (start > 0 || end < rawLines.length)) {
    return rawLines.map(parseLine).filter((x): x is ParsedItem => x !== null);
  }
  return parsed;
}
